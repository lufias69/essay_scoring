...
def essay_jaccard_similarity(kunci_jawaban, jawaban):
...
def essay_cosine_similarity(kunci_jawaban, jawaban):
    if type(kunci_jawaban) != list:
        kunci_jawaban = [kunci_jawaban]
    kunci_jawaban = cek_negasi_list(kunci_jawaban)
    kunci_jawaban_unik = " ".join(get_unik(kunci_jawaban))
...
def essay_dice_similarity(kunci_jawaban, jawaban):
    if type(kunci_jawaban) != list:
        kunci_jawaban = [kunci_jawaban]
    kunci_jawaban = cek_negasi_list(kunci_jawaban)
    kunci_jawaban_unik = " ".join(get_unik(kunci_jawaban))
...
def scoring_cosine(jawaban, kuncijawaban_list, skor_list, keterangan):
    ratio=list()
    tranform = list()
    for i in kuncijawaban_list:
        # rat = fuzz.token_set_ratio(i, gejala)
        rat = essay_cosine_similarity(i, jawaban)
        ratio.append(rat[0])
